...
class DGTrainer(Trainer):
    ...
    def predict(self, model, img):
        ...
        if h >= ps or w >= ps:
            ...
            pred_dmap = model(img)[0] if self.mode == 'base' else model(img)[0]
    ...
    def get_visualized_results_with_cls(self, model, img):
        ...
        if h >= ps or w >= ps:
            ...
            dmap,_,_,dmap1,dmap2,dmap3,cmap = model(img)

        dmap = dmap[0, 0].cpu().detach().numpy().squeeze()
        cmap = cmap[0, 0].cpu().detach().numpy().squeeze()
        dmap1 = dmap1[0, 0].cpu().detach().numpy().squeeze()
        dmap2 = dmap2[0, 0].cpu().detach().numpy().squeeze()
        dmap3 = dmap3[0, 0].cpu().detach().numpy().squeeze()

        return dmap,dmap1,dmap2,dmap3,cmap

    def generate_branch_masks(self, branch_predictions, y_mean):
        mask = (branch_predictions > y_mean).float()
    ...
    def train_step(self, model, loss, optimizer, batch, epoch):
        ...
        if self.mode == 'simple':
            optimizer.zero_grad()
            dmaps1,h,mean1,d11,d21,d31 = model(imgs1)
            # dmaps1,h = model(imgs1)
            mask11 = self.generate_branch_masks(d11,mean1)
            mask21 = self.generate_branch_masks(d21,mean1)
            mask31 = self.generate_branch_masks(d31,mean1)
      

            new_mask11 = mask11 * gt_dmaps
            new_mask21 = mask21 * gt_dmaps
            new_mask31 = mask31 * gt_dmaps
          
            loss_mask11 = self.compute_spilt_loss(loss, d11, new_mask11)
            loss_mask21 = self.compute_spilt_loss(loss, d21, new_mask21)
            loss_mask31 = self.compute_spilt_loss(loss, d31, new_mask31)
           
            loss_mask = loss_mask11+loss_mask21+loss_mask31
            loss_den = self.compute_count_loss(loss, dmaps1, gt_datas) 
            # loss_total = loss_den+h/2
            loss_total = loss_den+loss_mask+h
            loss_total.backward()
            optimizer.step()
            ...
            dmaps1,h1,mean1,d11,d21,d31 = model(imgs1)
            dmaps2,h2,mean2,d12,d22,d32 = model(imgs2)
            
            mask11 = self.generate_branch_masks(d11,mean1)
            mask21 = self.generate_branch_masks(d21,mean1)
            mask31 = self.generate_branch_masks(d31,mean1)
            mask12 = self.generate_branch_masks(d12,mean2)
            mask22 = self.generate_branch_masks(d22,mean2)
            mask32 = self.generate_branch_masks(d32,mean2)

            new_mask11 = mask11 * gt_dmaps
            new_mask21 = mask21 * gt_dmaps
            new_mask31 = mask31 * gt_dmaps
            new_mask12 = mask12 * gt_dmaps
            new_mask22 = mask22 * gt_dmaps
            new_mask32 = mask32 * gt_dmaps

            loss_mask11 = self.compute_spilt_loss(loss, d11, new_mask11)
            loss_mask21 = self.compute_spilt_loss(loss, d21, new_mask21)
            loss_mask31 = self.compute_spilt_loss(loss, d31, new_mask31)
            loss_mask12 = self.compute_spilt_loss(loss, d12, new_mask12)
            loss_mask22 = self.compute_spilt_loss(loss, d22, new_mask22)
            loss_mask32 = self.compute_spilt_loss(loss, d32, new_mask32)

            loss_mask = loss_mask11+loss_mask21+loss_mask31+loss_mask12+loss_mask22+loss_mask32
            ...
            optimizer.zero_grad()
            dmaps1, dmaps2, loss_con, (d11, d21, d31), (d12, d22, d32), (mean1, mean2),h = model.forward_train(imgs1,imgs2)
            ...
            loss_cls = (
                    F.binary_cross_entropy(cmaps31, gt_cmaps) +
                    F.binary_cross_entropy(cmaps32, gt_cmaps))

            loss_mask = loss_mask11 + loss_mask21 + loss_mask31 + loss_mask12 + loss_mask22 + loss_mask32

            loss_total = loss_den  + loss_mask+h+loss_cls
            ...
            optimizer.zero_grad()
            dmaps1,dmaps2,h1,h2, mean1,mean2,d11,d21,d31,d12,d22,d32,cmaps1,cmaps2,loss_con = model.forward_train(imgs1, imgs2, gt_cmaps)
            ...
            loss_total = loss_den + 10 * loss_cls+0.5*loss_mask +5*h+loss_con
            loss_total.backward()
            optimizer.step()


        elif self.mode == 'final':
            # gt_cmaps=F.interpolate(gt_cmaps, scale_factor=4, mode='nearest')
            optimizer.zero_grad()
            dmaps1, dmaps2, (mean1,m11,m21,m31,d11, d21, d31), (mean2,m12,m22,m32,d12, d22, d32), (cmaps1,cmaps2), loss_con, loss_err,h1,h2 = model.forward_train(
                imgs1, imgs2, gt_cmaps)
            mask11 = self.generate_branch_masks(m11, mean1)
            mask21 = self.generate_branch_masks(m21, mean1)
            mask31 = self.generate_branch_masks(m31, mean1)
            mask12 = self.generate_branch_masks(m12, mean2)
            mask22 = self.generate_branch_masks(m22, mean2)
            mask32 = self.generate_branch_masks(m32, mean2)
            ...
            loss_mask = (loss_mask11 + loss_mask21 + loss_mask31 + loss_mask12 + loss_mask22 + loss_mask32)/3
            ...
            loss_total = loss_den + 10 * loss_cls + loss_mask + 5 * h + 10 * loss_con   #//10  5 10 sota，
            loss_total.backward()
            optimizer.step()
    ...
    def compute_spilt_loss(self, loss: nn.Module, pred_dmaps, gt_datas, weights=None):
        if loss.__class__.__name__ == 'MSELoss':
            if weights is not None:
                pred_dmaps = pred_dmaps * weights
                gt_datas = gt_datas * weights
            loss_value = loss(pred_dmaps, gt_datas * self.log_para)
    ...
    def vis_step(self, model, batch):
        ...
        if self.mode == 'base':
            ...
            pred_dmap1,dmap11,dmap21,dmap31,pred_cmap1 = self.get_visualized_results_with_cls(model, img1)
            pred_dmap2,dmap12,dmap22,dmap32,pred_cmap2 = self.get_visualized_results_with_cls(model, img2)
            img1 = denormalize(img1.detach())[0].cpu().permute(1, 2, 0).numpy()
            img2 = denormalize(img2.detach())[0].cpu().permute(1, 2, 0).numpy()
            pred_count1 = pred_dmap1.sum() / self.log_para
            pred_count2 = pred_dmap2.sum() / self.log_para
            count_1 = dmap11.sum() / self.log_para
            count_2 = dmap21.sum() / self.log_para
            count_3 = dmap31.sum() / self.log_para
            ...
            new_datas = [img1, pred_dmap1,dmap11,dmap21,dmap31]
            new_titles = [f'{name[0]}', f'Pred_{pred_count1}',f'1_{count_1}',f'2_{count_2}',f'3_{count_3}']
