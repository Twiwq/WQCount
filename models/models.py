...
# 上采样操作
def upsample(x, scale_factor=2, mode='bilinear'):  # scale_factor是扩大倍数
    if mode == 'nearest':
...
class DGModel_base(nn.Module):  # 基本模型，包括特征提取（使用预训练的 VGG16）、解码器、密度图生成器等。上采样用于恢复图像的空间分辨率。
    def __init__(self, pretrained=True, den_dropout1=0.3,den_dropout2=0.3, den_dropout3=0.3):  # pretrained控制是否使用预训练的 VGG16 模型权重。
        super().__init__()
    ...
    def forward_similarity(self, y1, y2, y3):
        ...
        score1 = (logits_12 + logits_13) / 2  # 特征图 1 的相似度得分
        score2 = (logits_12 + logits_23) / 2  # 特征图 2 的相似度得分
        score3 = (logits_13 + logits_23) / 2  # 特征图 3 的相似度得分
    ...
    def compute_attention_weights(self, feature_map):
        ...
        # 计算权重，直接除以阈值
        weights = feature_map / (thresholds + 1e-8)  # 防止除以零
    ...
    def forward(self, x):
        ...
        # y1_weighted = upsample(y1_weighted, scale_factor=4)
        # y2_weighted = upsample(y2_weighted, scale_factor=4)
        # y3_weighted = upsample(y3_weighted, scale_factor=4)

        # d_mean = upsample(d_mean, scale_factor=4)
...
class DGModel_mem(DGModel_base):  # 在 DGModel_base 的基础上增加了通过记忆库进行特征增强
    ...
    def forward(self, x):
        ...
        # d2 = self.den_head1(y_den2)
        # d3 = self.den_head1(y_den3)
...
class DGModel_memadd(DGModel_mem):
    ...
    def forward_train(self, img1, img2):
        ...
        # 绝对误差小的时候，参数掩码如果不对误差较小的区域进行掩码处理，模型可能会对这些区域进行过度拟合
